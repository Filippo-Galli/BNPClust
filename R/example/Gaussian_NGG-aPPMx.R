...
#source("R/distributionalStats_retrieval/LA.R")

location <- "LA"
input_folder <- paste0("input/", location, "/")
df <- read.csv(paste0(input_folder, "full_dataset.csv"))
W <- as.matrix(readRDS(paste0(input_folder, "adj_matrix.rds")))

puma_ids <- sort(unique(df$COD_PUMA))
income_by_puma <- split(df$log_income, df$COD_PUMA)[as.character(puma_ids)]

max_len <- max(lengths(income_by_puma))
data_mat <- matrix(NA_real_, nrow = length(income_by_puma), ncol = max_len)

for (i in seq_along(income_by_puma)) {
    x <- income_by_puma[[i]]
    data_mat[i, seq_along(x)] <- x
}
##############################################################################
# Data Loading ====
##############################################################################
...
file_chosen <- files[4]
data_matrix <- as.matrix(data_mat)
...
print("Process param instantiated")
utils_param <- bnp_mod$create_utils_params(5000, 15000, data_matrix)
print("Utils param instantiated")

obs <- data_matrix[!is.na(data_matrix)]
m0 <- mean(obs, na.rm = TRUE)
kappa0 <- 0.01
alpha0 <- 2
beta0 <- var(obs, na.rm = TRUE) * (alpha0 - 1)
...
run_method <- "SM25+Gibbs1"
run_init <- "kmeans"
run_label <- "GaussianMixture"
