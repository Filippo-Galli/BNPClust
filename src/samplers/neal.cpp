...
int Neal3::sample_from_log_probs(int num_clusters) {
  ...
  double max_loglik = *std::max_element(log_likelihoods.begin(),
                                        log_likelihoods.begin() + num_clusters);
  ...
  // Fallback for floating point inaccuracies: pick the highest probability
  // cluster
  return std::distance(
      weights.begin(),
      std::max_element(weights.begin(), weights.begin() + num_clusters));
}
...
void Neal3::step_1_observation(int index) {
  ...
  for (int k = 0; k < K; ++k) {
    ...
    log_likelihoods[k] =
        likelihood.point_loglikelihood_cond(index, k) + log_prior;
  }

  // Compute for new cluster
  log_likelihoods[K] = likelihood.point_loglikelihood_cond(index, K) +
                       process.gibbs_prior_new_cluster_obs(index);
