...
private:
   ...
   */
  ClusterStats
  compute_stats(const Eigen::Ref<const Eigen::VectorXi> &cls_ass_k) const;
  ...
  /**
   * @brief Compute the log predictive likelihood for a given point and cluster
   * assignment.
   *
   * @param stats ClusterStats struct containing the computed statistics
   * @param x Point value
...
public:
   ...
   * @param params GaussianMixtureModel_params struct containing model
   * parameters
   */
  ...
  /**
   * @brief Compute the log likelihood of a cluster given its assignment and
  ...
  double cluster_loglikelihood(
      int cluster_index,
      const Eigen::Ref<const Eigen::VectorXi> &cls_ass_k) const override;
