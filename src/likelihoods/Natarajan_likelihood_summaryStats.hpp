/**
 * @file Natarajan_likelihood_summaryStats.hpp
...
/**
 * @class Natarajan_likelihood_summaryStats
 * @brief Computes log-likelihood for clusters based on distance-based cohesion
 * and repulsion using summary statistics for the repulsion term
 *
 ...
 * point-level conditional log-likelihoods. The difference between this class
 * and the original Natarajan_likelihood class is that this class uses summary
 * statistics instead of the full distance matrix for the repulsion term.
 * reference: Natarajan et al. (2023) "Cohesion and Repulsion in Bayesian Distance Clustering"
 *
 */
class Natarajan_likelihood_summaryStats : public Likelihood {
private:
    ...
    double lgamma_zeta_mt_cache;               ///< Precomputed lgamma(zeta_mt) for repulsion
     ...
     */
    double compute_cohesion(int point_index, int cluster_index, const Eigen::Ref<const Eigen::VectorXi> &cls_ass_k,
                            int n_k) const;
     ...
     */
    double compute_repulsion(int point_index, int cluster_index, const Eigen::Ref<const Eigen::VectorXi> &cls_ass_k,
                             int n_k) const;
...
public:
     ...
     */
    Natarajan_likelihood_summaryStats(const Data &data, const Params &param)
        : Likelihood(data, param), lgamma_delta1(lgamma(params.delta1)),
          log_beta_alpha(log(params.beta) * params.alpha - lgamma(params.alpha)), lgamma_delta2(lgamma(params.delta2)),
          log_gamma_zeta(log(params.gamma) * params.zeta - lgamma(params.zeta)), D_cols(params.D.cols()) {
        ...
        lgamma_zeta_mt_cache = lgamma(params.zeta + params.delta2);
    }
