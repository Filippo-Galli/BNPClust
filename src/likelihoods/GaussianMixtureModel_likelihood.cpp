...
GaussianMixtureModel_likelihood::GaussianMixtureModel_likelihood(const Data &data,
                                                                 const GaussianMixtureModel_params &params)
    : Likelihood(data), params(params), y(params.y), log_kappa0(std::log(params.kappa0)),
      lgamma_alpha0(std::lgamma(params.alpha0)), alpha0_log_beta0(params.alpha0 * std::log(params.beta0)),
      log_pi(std::log(M_PI)) {
    if (y.size() != data.get_n()) {
        throw std::invalid_argument("GaussianMixtureModel_likelihood: y size must match data.n");
    }
    if (params.kappa0 <= 0.0 || params.alpha0 <= 0.0 || params.beta0 <= 0.0) {
        throw std::invalid_argument("GaussianMixtureModel_likelihood: kappa0, alpha0, beta0 must be positive");
    }
}

GaussianMixtureModel_likelihood::ClusterStats
GaussianMixtureModel_likelihood::compute_stats(const Eigen::Ref<const Eigen::VectorXi> &cls_ass_k) const {
    ClusterStats stats;
    stats.n = cls_ass_k.size();
    for (int i = 0; i < stats.n; ++i) {
        const int idx = cls_ass_k(i);
        const double val = y(idx);
        stats.sum += val;
        stats.sumsq += val * val;
    }
    return stats;
...
double GaussianMixtureModel_likelihood::log_marginal_likelihood(const ClusterStats &stats) const {
    const int n = stats.n;
    if (n == 0) {
        return 0.0;
    }
    ...
    const double beta_n =
        params.beta0 + 0.5 * sse + 0.5 * (params.kappa0 * static_cast<double>(n) / kappa_n) * mean_diff * mean_diff;
...
double GaussianMixtureModel_likelihood::log_predictive_from_stats(const ClusterStats &stats, double x) const {
...
double GaussianMixtureModel_likelihood::cluster_loglikelihood(int cluster_index) const {
    auto cls_ass_k = data.get_cluster_assignments_ref(cluster_index);
    return cluster_loglikelihood(cluster_index, cls_ass_k);
}

double
GaussianMixtureModel_likelihood::cluster_loglikelihood(int cluster_index,
...
double GaussianMixtureModel_likelihood::point_loglikelihood_cond(int point_index, int cluster_index) const {
    const double x = y(point_index);
